export function createUI({ document, window, canvas, simulation, viewState, store,
  ...
  function makeLineageLink(plant) {
    ...
    link.title = plant.alive
      ? `gen ${plant.generation}, тик ${plant.bornAt}`
      : `gen ${plant.generation}, тик ${plant.bornAt} – ${plant.diedAt}`;
  ...
  function renderGenomeList() {
    ...
    if (entries.length === 0) {
      list.innerHTML =
        '<span style="color:#555;font-size:12px">Нет сохранённых</span>';
      return;
    }
  ...
  document.getElementById("btn-save-genome").addEventListener("click", () => {
    const name = window.prompt("Имя генома:",
